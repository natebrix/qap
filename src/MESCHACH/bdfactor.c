...
int new_lb,new_ub,new_n;
{
   ...
   new_lb = A->lb = imin(new_lb,new_n-1);
   new_ub = A->ub = imin(new_ub,new_n-1);
...
MAT *A;
{
   ...
   for (j=0; j < n; j++)
     for (i=imin(n1,j+lb),l=lb+j-i; i >= imax(0,j-ub); i--,l++)
       A->me[i][j] = bmat[l][j];
...
BAND *in, *out;
{
   ...
   if (! in_situ) {
	 ...
	 **********************************/
	 ...
	 lbi = lb-i;
	 for (j=l-lb, jj=0, p=imax(-lbi,0), pp = imax(l-ub,0); j <= n1; 
	      j++,jj++,p++,pp++) {
	    in_v[l][pp] = in_v[i][p];
	    in_v[i][jj] = in_v[l][j];
	 }
	 for (  ; p <= n1 - imax(lbi,0); p++,pp++)
	   in_v[l][pp] = in_v[i][p];
      }
      
      if (lub%2 == 0) { /* shift only */
	 i = lub/2;
	 for (j=imax(i-lb,0), jj=0; jj <= n1-ub+i; j++,jj++) 
	   in_v[i][jj] = in_v[i][j];
      }
   }
      ...
      for (i=0, l=lub; i < (lub+1)/2; i++,l--) {
	 ubi = i-ub;
	 for (j=n1-imax(lb-l,0), jj=n1-imax(-ubi,0), p=n1-lb+i, pp=n1;
	      p >= 0; j--, jj--, pp--, p--) {
	    in_v[i][jj] = in_v[l][j];
	    in_v[l][pp] = in_v[i][p];
	 }
	 for (  ; jj >= imax(ubi,0); j--, jj--)
	   in_v[i][jj] = in_v[l][j];
      }

      if (lub%2 == 0) {  /* shift only */
	 i = lub/2;
	 for (j=n1-lb+i, jj=n1-imax(ub-i,0); j >= 0; j--, jj--) 
	    in_v[i][jj] = in_v[i][j];
      }
   }
...
PERM	*pivot;
{
   ...
   bA = bd_resize(bA,lb,imin(n1,lub),n);
   bA_v = bA->mat->me;
...
VEC	*b,*x;
{
   ...
   for (i=n-2; i >= 0; i--) {
      c = x->ve[i];
      for (j=imin(n1,i+ub), l=lb+j-i; j > i; j--,l--)
	c -= bA_v[l][j]*x->ve[j];
      x->ve[i] = c/bA_v[lb][i];
   }
...
BAND *A;
{
   ...
   for (k=0; k < n; k++) {    
      ...
      /* matrix D */
      c = Av[lb][k];
      for (j=imax(0,-lbkm), jk=lbkm+j; j < k; j++, jk++) {
	 cc = Av[jk][j];
	 c -= Av[lb][j]*cc*cc;
      }
      if (c == 0.0)
	error(E_SING,"bdLDLfactor");
      ...
      for (i=imin(n1,lbkp), ki=lbkp-i; i > k; i--,ki++) {
	 c = Av[ki][k];
	 for (j=imax(0,i-lb), ji=lb+j-i, jk=lbkm+j; j < k;
	      j++, ji++, jk++)
...
VEC    *b, *x;
{
   ...
   for (i=1; i < n; i++) {
      ilb = i-lb;
      c = b->ve[i];
      for (j=imax(0,ilb), l=j-ilb; j < i; j++,l++)
	c -= Av[l][j]*x->ve[j];
      x->ve[i] = c;
   }
   ...
   for (i=n-2; i >= 0; i--) {
      ilb = i+lb;
      c = x->ve[i];
      for (j=imin(n1,ilb), l=ilb-j; j > i; j--,l++)
	c -= Av[l][i]*x->ve[j];
      x->ve[i] = c;
   }
